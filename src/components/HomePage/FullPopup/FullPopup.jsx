...
// 더미 데이터 정의
const DUMMY_DATA = {
  questionImg: '/api/placeholder/400/300',
  answer: '42',
  memo: `Step 1: 주어진 식을 분해합니다
$3x + 4 = 10$
$3x = 6$
...
$6 + 4 = 10$
$10 = 10$ ∴ 성립`,
  note: ['/api/placeholder/400/300']
};

const FullPopup = ({ userQuestionId, onClose }) => {
  const [activeTab, setActiveTab] = useState('memo');
  const [isLoading, setIsLoading] = useState(false); // 더미 데이터 사용시 false로 설정
  const [questionData, setQuestionData] = useState(DUMMY_DATA);
  const [error, setError] = useState(null);

  // API 호출 부분은 주석 처리 (필요시 활성화)
  /*
  useEffect(() => {
    const loadQuestionData = async () => {
      try {
