...
function AnswerModal({ isOpen, isCorrect, onClose, onExplain }) {
  if (!isOpen) return null; // 모달이 열리지 않았다면 렌더링하지 않음

  return (
    <div className={styles.modalOverlay}>
      <div className={styles.modalContent}>
        <p className={styles.modalText}>{isCorrect ? '축하합니다!' : '아쉽지만, 더욱 발전할 수 있어요!'}</p>
        <div className={styles.buttonContainer}>
          <CustomButton size="medium" color="gray" type="filled" text="돌아가기" onClick={onClose} />
          <CustomButton size="medium" color="blue" type="filled" text="풀이 보러가기" onClick={onExplain} />
        </div>
      </div>
    </div>
  );
