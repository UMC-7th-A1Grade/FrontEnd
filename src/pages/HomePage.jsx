...
const HomePage = () => {
 ...
 useEffect(() => {
   const checkLoginStatus = async () => {
     try {
       // 두 API를 동시에 호출
       const [questionsResponse, creditResponse] = await Promise.allSettled([
         getRecentQuestions(),
         getUserCredits()
       ]);

       // 두 API 모두 500 에러인지 확인
       const isBothError = 
         questionsResponse.status === 'rejected' && 
         creditResponse.status === 'rejected' &&
         questionsResponse.reason?.response?.status === 500 &&
         creditResponse.reason?.response?.status === 500;
